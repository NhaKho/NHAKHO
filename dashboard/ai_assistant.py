...
def ask_ai(question: str, data_summary: str, con, history=None):
    ...
    router_prompt = [
        {"role": "system", "content":
            ...
            "'video đó', 'kênh này', 'cái đó', 'so với trên'..., hãy dựa vào các lượt hội thoại trước đó "
            "(bên dưới) để suy ra chính xác video/kênh/danh mục đang được nhắc tới, rồi mới áp dụng "
            "các quy tắc 1-4 tiếp theo. Nếu không đủ ngữ cảnh để suy ra, cứ viết SQL truy vấn theo cách "
            "hiểu hợp lý nhất, không được bỏ qua câu hỏi.\n"
